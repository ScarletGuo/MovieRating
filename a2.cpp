...
void fillDictionary(std::ifstream &newInFile,
	...
	while (getline(newInFile, line)) {
		std::vector<std::string> words;
		splitLine(line, words);
		std::long rating = atol(words[0].c_str());
		std::long COUNTER = 1;
		for (int i = 1; i < words.size(); i++) {
			if (dict.find(words[i]) == dict.end()) { // not exist 
				dict.emplace(words[i],make_pair(rating, COUNTER));
			} else { // already exist
				std::long rate, count;
				rate = dict[words[i]].first + rating;
				count = dict[words[i]].second + COUNTER;
				dict[words[i]] = make_pair(rate, count);
			}	
		}	
	}
}
